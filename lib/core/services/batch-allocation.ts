...
export class BatchAllocationService {
  ...
  async createBatch(
    ...
    const insertResult = await db.insert(taskBatches).values({
      name,
      description,
      targetUserCount,
    ...
    // Get the inserted batch ID from MySQL driver response
    const insertId = (insertResult as any)[0].insertId;
    const [batch] = await db.select().from(taskBatches).where(eq(taskBatches.id, insertId));
