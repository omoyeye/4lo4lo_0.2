...
export class AnalyticsService {
  ...
  async updateUserEngagementScore(userId: number) {
    try {
      ...
      const analytics = await db.select()
        ...
        .where(and(
          eq(userAnalytics.userId, userId),
          gte(userAnalytics.date, last30Days.toISOString().split('T')[0])
        ));
  ...
  async getDashboardMetrics() {
    try {
      ...
      const activeUsersToday = await db.select({ count: count() })
        .from(userAnalytics)
        .where(eq(userAnalytics.date, today));
      ...
      const recentAnalytics = await db.select()
        .from(userAnalytics)
        .where(gte(userAnalytics.date, last7Days.toISOString().split('T')[0]));
  ...
  async getUserEngagementTrends(days: number = 30) {
    try {
      ...
      .from(userAnalytics)
      .where(gte(userAnalytics.date, startDate.toISOString().split('T')[0]))
  ...
  async getTaskPerformanceAnalytics(taskId?: number) {
    try {
      ...
      if (taskId) {
        query = query.where(eq(batchTaskAllocations.taskId, taskId));
      }
